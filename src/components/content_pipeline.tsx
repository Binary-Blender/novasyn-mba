...
export function content_pipeline() {
  const items = useAppStore(s => s.contentPipelineItems ?? []);

  const columns = Array.from(new Set(items.map((i: any) => i.status ?? 'unknown')));
